...
<body>
<div class="example">
    <h1 class="text-center">销售核对</h1>
    <div class="panel">
        <div class="panel-heading">
           门店:
            @if(isset($user->store->name))
            {{$user->store->name??''}}
            @else
                <from  id="info">
                    @csrf
{{--                    <input type="text" name="name" value="11" id="store">--}}
                <div class="input-group" >
                    ...
                    <button class="btn btn-default" type="button" onclick="btn()" >保存</button>
                  </span>
                </div>
                </from>
            @endif
            (来自群：{{ $img->from_group_name }}   促销员:{{ $img->from_name }})
        </div>
        <div class="panel-body">
            <div class="abstract">
                <form class="form-inline" method="POST" action="{{ route('addsale',$img) }}">
                    @csrf
                    <div class="form-group">
                        <label for="exampleInputEmail3">型号</label>
                        <input name="model" type="text" class="form-control" placeholder="型号" id="myInput" class="@error('model') is-invalid @enderror">
                        @error('model')
                        <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="form-group">
                        <label for="exampleInputInviteCode3">提成</label>
                        <input name="ticheng" type="text" class="form-control" placeholder="提成" class="@error('ticheng') is-invalid @enderror">
                        @error('ticheng')
                        <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    <input type="hidden" name="store_id" value="{{$user->zt_store_id}}">
...
<script>
    function btn() {
        ...
        $.ajax({
            ...
            type:'POST',//提交类型
            dataType:'json',//数据形式
            url:"{{ route('upstore',$user->id)}}",//提交的url
            data:data,//序列化为可以传输的数据
