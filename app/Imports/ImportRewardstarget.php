...
class ImportRewardstarget implements ToModel, WithValidation, SkipsOnFailure
{
    use Importable;
    ...
    public function model(array $row)
    {
        ...
        return new ZtReward([
            'model' => $row[0],
            'reward' => $row[1],
            'starttime' => date("Y-m-d H:i:s",formatTime($row[2])),
            'endtime' => date("Y-m-d H:i:s",formatTime($row[3])),
            'state' => 1,
        ]);
    }
